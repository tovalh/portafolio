    ...
    return (
        <div className={`relative w-full md:w-[45%] ${isLeft ? 'md:mr-auto' : 'md:ml-auto'}`}>
            <div className={`bg-white/70 dark:bg-white/10 backdrop-blur-md p-6 rounded-3xl border transition-all duration-300 hover:shadow-xl relative hover:-translate-y-1
                ${item.highlight ? 'border-primary/30 shadow-primary/10' : 'border-white/50 dark:border-white/20 shadow-sm'}
            `}>

                {/* Connector Line to center */}
                <div className={`absolute top-6 w-8 h-0.5 bg-dark/20 dark:bg-white/20 hidden md:block
                        ...
                        {TIMELINE_DATA.map((item, index) => {
                            ...
                            return (
                                <div key={item.id} className="relative flex items-start pl-12 md:pl-0">
                                    ...
                                    {/* Center Icon */}
                                    <div className={`absolute left-0 md:left-1/2 top-6 flex items-center justify-center w-10 h-10 md:w-12 md:h-12 rounded-full bg-background shadow-lg z-20 transform md:-translate-x-1/2`}>
                                        {item.type === 'work' ? (
